        ...
        AddressService addressService)
    {
        ...
        private Message<string>? RejectWrongApp(AppUser user)
        {
            if (string.Equals(user.UserType, "Worker", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Status = "E",
                    Text = "This account is a Worker. Please use the Worker app.",
                    Code = "WRONG_APP",
                    Result = string.Empty
                };
            }
            ...
            if (string.Equals(user.UserType, "Admin", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    Status = "E",
                    Text = "Admin accounts cannot sign in to the User app.",
                    Code = "WRONG_APP",
                    Result = string.Empty
                };
            }
        ...
        private async Task<(string? Otp, Message<string>? Error)> SendOtpAsync(string normalizedPhone)
        {
            ...
            {
                var smsText =
                    $"Welcome to Onee! Your verification code is {otp}. This code expires in {OtpExpiryMinutes} minutes. Do not share it with anyone.";
            ...
            string otp)
        {
            ...
            if (isNew && user == null)
            {
                // Temporary username until register sets email as UserName.
                user = new AppUser
                {
        ...
        public async Task<Message<string>> SetOnlineStatusAsync(string userId, bool isOnline)
        {
            ...
            {
                Status = "S",
                Text = isOnline ? "User is now online." : "User is now offline.",
        ...
        public async Task<FELoggedUserDetailsModel?> GetUserDetailsAsync(string id)
        {
            ...
            return new FELoggedUserDetailsModel
            {
                ...
                ProImg = CommonResources.BuildUploadUrl(uploadPath, "User", user.ProfileImageUrl),
