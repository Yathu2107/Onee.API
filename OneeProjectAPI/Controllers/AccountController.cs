...
namespace OneeProjectAPI.Controllers
{
    ...
    public class AccountController(AccountService accountService, UserManager<AppUser> userManager) : ControllerBase
    {
        ...
        public async Task<IActionResult> Login([FromBody] TokenRequestModel model)
        {
            ...
            if (string.IsNullOrEmpty(authModel.Token) && !authModel.ForcePasswordReset)
            {
                return BadRequest(new Message<AuthenticationModel>
                {
                    Status = "E",
            ...
            // Case 2: Force password reset (auto-generated password)
            if (authModel.ForcePasswordReset)
            {
                return Ok(new Message<AuthenticationModel>
                {
                    Status = "S",
                    Text = authModel.Message, // "Password reset required"
                    Code = "FORCE_RESET",
                    Result = authModel
                });
            }

            // Case 3: Successful login, set refresh token cookie
        ...
        [AllowAnonymous]
        [HttpPost]
        [Route(API_ROUTE_NAME + "/reset-auto-password")]
        public async Task<IActionResult> ResetAutoGeneratedPassword([FromBody] ResetPasswordRequest model)
        {
            if (!ModelState.IsValid)
                return BadRequest(new Message<string>
                {
            ...
            var result = await _accountService.ResetAutoGeneratedPasswordAsync(model.Email, model.NewPassword);
